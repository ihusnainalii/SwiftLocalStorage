...
### Fixed
- CI: the package builds with Xcode 16.4 / Swift 6.1 again (SwiftData `MigrationStage` is not `Sendable`, so the schema migration stages are computed instead of stored statics); sources and tests pass SwiftLint and swift-format in strict mode.
